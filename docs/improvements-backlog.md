...
- **Atualização (2026-09-11, story `statistics-rpc-integration-tests`):** confirmado na prática — `supabase db reset` local com as 277 migrations do repo **quebra** na 57 (`20260324200000_fix_threecplus_connections_columns`: renomeia `extension`, mas o create `20260324170000` foi editado depois e já nasce com `login`) e na 108 (`20260331080000_enable_rls_ldr_tables`: `ldr_empresas` só existe em prod); prod tem ~330 migrations registradas. Contorno adotado para as funções de estatística: **banco de teste enxuto** (`tests/integration/fixtures/statistics-schema.sql` copiado de prod + as migrations das funções). Continua bloqueado por este item: rodar `tests/integration/rls-policies.test.ts` no CI e qualquer ambiente local/preview fiel a prod. Planos existentes: `docs/migration-history-reconciliation-plan.md` (30/mai) e `docs/db-migration-drift-reconciliation-plan.md` (08/jun).
